...
-- Create a table for shared nuggets
create table shared_nuggets (
  id bigint generated by default as identity primary key,
...
create index idx_shared_nuggets_share_id on shared_nuggets(share_id);

-- Set up RLS for shared_nuggets
alter table shared_nuggets enable row level security;

-- Anyone can read shared nuggets (public)
create policy "Shared nuggets are publicly viewable"
  on shared_nuggets for select
  using ( true );

-- Only creators can insert/delete their shares
